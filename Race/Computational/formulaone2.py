...
def how_long_tyres(tyre_tol):
    ...
    for l in range(totalLaps):
        ...
        if tyre_performance(l,"hard")<tyre_tol:
            hard_l_max+=1
    return [soft_l_max,med_l_max,hard_l_max]
#Acceptable codes:
...
if __name__ == "__main__":
    ...
    bestTyres = []
    t_list = []
    tyre_tols = np.linspace(0.5,2.5,21)
    for i in tyre_orders:
        for j in tyre_orders:
            tyre_order = [i[0],i[1],i[2],j[0],j[1],j[2]]
            for tyre_tol in tyre_tols:
                [soft_l_max,med_l_max,hard_l_max] = how_long_tyres(tyre_tol)
                ############################################################################
                #Starting Tyres
                ############################################################################
    ...
    print("The tyre tolerance is",bestTyreTol)
